...
contract VRFConsumerV2 is VRFConsumerBaseV2, Executor {
    ...
    function calculatePremiumAmount(uint256 startAmount, uint256 endAmount, uint256 limit) internal returns (uint256 amount) {
        ...
        assembly {
            ...
            amount := div(div(div(diff, 2), local_precision), local_demonator)
        }
    }
    ...
    function betainv(uint256 x, uint256 requestId) internal returns (uint256, uint256) {

        // x已经调整到0-1000范围,直接查表
        uint256 mod_x = x % (precision+1);

        uint256 limit = limits[requestId];
